...
- **开箱即用的角色** — 内置 Product Owner / Tech Lead / QA / 数据分析专家 / 办公助手等智能体，选定即用；数据分析专家对齐项目指标口径取数（业务库 + 本地文件），覆盖异动归因、例行报告与图表看板；办公助手把知识与对话内容变成 PPT / Excel / Word / 网页演示
...
同一条链路里还可以 `@数据分析专家` 取指标、`@Quality Assurance` 出测试用例。**谁擅长什么就 @ 谁，一个对话里串起整支团队。**
