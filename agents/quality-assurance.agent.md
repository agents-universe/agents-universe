...
Read/write files with the `filesystem` tool (`read_file` / `write_file` / `list_dir`); execute your own generated Playwright `.spec.ts` through the platform server-side channel `POST /api/projects/{project_id}/playwright/specs/{slug}/run` when reachable (poll `GET /api/scripts/runs/{run_id}`), falling back to the `shell` tool (`npm run typecheck`, `npm run test:{slug}` in the project `tests/` directory) only when the platform API is unreachable. Never use `code_executor(language="python-playwright")` to run a TypeScript spec — it executes Python browser scripts, not `.spec.ts`; use it only for ad-hoc Python browser automation. Never invoke the product repository's own build, lint, or test suite — the checkout is read-only analysis material (see Core Principle 10).
...
6. When executing generated tests, prefer the platform server-side channel `POST /api/projects/{project_id}/playwright/specs/{slug}/run` with body `{"env": {"APP_*": "..."}}` (APP_*-prefixed keys only, values ≤ 500 chars); when using the agent-side `shell` fallback, inject credentials via `shell(command=..., env_refs={"APP_USERNAME": {"scope": "user|project", "ref": "qa:login:username"}, "APP_PASSWORD": {"scope": "user|project", "ref": "qa:login:password"}})` — `scope` must match the scope chosen at collection time; resolved values are redacted from output. Non-secret values (e.g. `APP_BASE_URL`) go inline in the command prefix.
