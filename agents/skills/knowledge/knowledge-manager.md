...
| Slug (path under `knowledge/`, no `.md`) | Contents | Update Frequency |
|------|----------|------------------|
| `domain/context` | Overall background, business goals, architecture summary | Establish early, update infrequently |
| `domain/glossary` | Domain terms and abbreviations | Append when new terms appear |
| `technical/login-and-user-switch` | Login entry points, user/company/tenant switching modes, verified accounts | After login-flow or permission corrections |
| `technical/page-map` | Page routes -> functional module mapping | When requirements change |
| `skills/ui-patterns` | Verified UI selectors and interaction patterns | After each testing correction |
| `technical/api-map` | Product-owned APIs, entry structure (index only — detail schemas in `technical/api/*.md`) | After OpenAPI/service discovery; **MUST also create/update detail files** |
...
| `technical/data-model` | Core tables with layer/granularity, fact-dimension relationships, data quality rules | When table model or quality rules change |
| `technical/data-pipelines` | ETL/ELT job inventory, schedules, dependencies, backfill and failure handling | When jobs or schedules change |
| `domain/metric-catalog` | Authoritative metric definitions (formula, dimensions, source tables) and conflict rulings | When metric definitions are added/changed or conflicts ruled |
| `domain/analysis-scenarios` | Recurring reports/dashboards inventory and ad-hoc analysis log | When reports change; append after each thematic analysis |
| `skills/sql-patterns` | SQL dialect/engine, conventions, reusable snippets (retention/funnel/sessionization), performance notes | Extract after recurring query patterns are verified |
| `skills/analysis-patterns` | Analysis method selection, chart selection rules, report structure | When methodology conventions are established or corrected |
| `domain/faq` | Customer-service FAQ: one Q&A per topic, question-formatted headings, negative examples ("we do not provide X"); answers must trace to service policies | After each new/corrected Q&A or answer correction |
...
- Execution evidence (test results, screenshots, log output) — but API contract facts learned by probing (required fields, value constraints, correct paths, idempotency behavior) and verified data-setup recipes **do qualify**: they belong to the *API contracts* / *Test patterns* rows above, not to this exclusion.
...
- **Updating an established file** (adding a selector to `skills/ui-patterns`, correcting a route in `technical/page-map`, appending a new API endpoint to `technical/api-map`): lightweight check — if the update fits the file's declared category above, proceed.
...
Slugs and file names stay **ASCII** — `KNOWLEDGE_SLUG_RE` rejects CJK path segments in `knowledge_rw`. A Chinese display name goes in frontmatter `title` (e.g. `title: "API 详情"`); never create a file named `api详情.md`.
...
2. **Create the index file**: `knowledge_level: root` at the original slug. Body = summary table/list with one-line descriptions of each child.
3. **Create detail files**: Per sub-section, `knowledge_level: detail` with `parent` pointing to the index slug.
4. **Update the index's `children` list** to list all child slugs (`knowledge_rw write` keeps the parent's `children` in sync automatically when the child declares `parent`).
5. **Verify symmetry**: Every child's `parent` must match the index; the index's `children` must list every child.
6. **Append to `system/history`**: Record the hierarchy creation.
...
**Level 1 — `technical/api-map` (primary, auto-loaded)**
...
**FORBIDDEN in technical/api-map:** request bodies, response bodies, field schemas, error code tables, parameter details. These MUST go in level-2 files.
...
2. [ ] `technical/api-map` frontmatter `children` list includes all child slugs
3. [ ] `technical/api-map` body has `[[technical/api/{service-slug}]]` links for every service
4. [ ] Each detail file has `parent: "technical/api-map"` in frontmatter
5. [ ] `system/history` records the hierarchy creation

If you write API content to `technical/api-map` without creating the corresponding detail files, the system will warn you and the task is considered INCOMPLETE.

The same pattern applies to `technical/kong-map` — if Kong route details (full request/response mappings) grow large, split into `technical/kong/{route-group}.md` detail files with `parent: "technical/kong-map"`.

### Refreshing / Relearning API Documentation

When the user asks to refresh, relearn, or re-fetch Swagger/API docs, you MUST:

1. **Update `technical/api-map`** — refresh the service catalog, endpoint list, counts
...
4. **Remove stale entries** — mark defunct services' detail files `[stale]` and drop them from `technical/api-map` children

A refresh that only updates `technical/api-map` without touching detail files is INCOMPLETE — the detail files hold the actual schemas used for test generation and API interaction; they must stay in sync.

**Quick check after refresh:**
- `ls knowledge/technical/api/` should have one `.md` file per service listed in `technical/api-map`
- Each detail file reflects the latest OpenAPI spec
- `technical/api-map` `children` list matches the actual files
