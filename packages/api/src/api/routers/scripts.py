...
# WS log poll cadence and total window. 900s covers a cold Playwright run
...
def _persist_run_log(run: ScriptRun, log_acc: list[str]) -> None:
    ...
    run.stdout_log = "".join(log_acc)[:_LOG_CAP]


async def _drain_stream(stream, log_acc: list[str], budget: list[int]) -> None:
...
async def _stream_subprocess(
    db: AsyncSession, run: ScriptRun, log_acc: list[str],
    ...
    budget = [_LOG_CAP - sum(len(s) for s in log_acc)]
    readers = [
        asyncio.create_task(_drain_stream(proc.stdout, log_acc, budget)),
        asyncio.create_task(_drain_stream(proc.stderr, log_acc, budget)),
    ]
    ...
    async def _flush_loop() -> None:
        while not stop.is_set():
            try:
                await asyncio.wait_for(stop.wait(), timeout=_RUN_POLL_INTERVAL)
...
async def _execute_script(
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            exit_code = await _stream_subprocess(
                db, run, log_acc, cmd, str(project_root), env, 300,
            )
