...
describe('inferTier — Chinese domestic brands (conservative mid default)', () => {
  it('brand defaults and explicit keywords', () => {
    ...
    // Multi-segment ids, and 5.10 above 5.3 (parseFloat would read 5.1).
    expect(inferTier('openai', 'glm-5.3.1')).toBe('high')
    expect(inferTier('openai', 'glm-4.5.2')).toBe('mid')
    expect(inferTier('openai', 'glm-5.10')).toBe('high')
