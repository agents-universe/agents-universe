...
- **Path B — no demo**:
  ...
  3. Otherwise **`delegate_agent` the style investigation to `quality-assurance`** (one tool call, inside this turn — the result comes back to you and you keep working; do not end the turn asking the user to @-mention QA). QA reads CSS sources first, extracts computed styles (`getComputedStyle`) second, and screenshots with annotations last; QA returns a compact baseline list (palette with contrast hints, font stack/sizes/weights, button/card/table/nav/tab styles, layout, hover/active/focus states) and appends it to `knowledge/ui-patterns.md` under a **Visual Style Baseline** section (`[verified]` from real system/computed styles, `[inferred]` from estimation) plus `history.md`. The delegation result carries the baseline and the deposit confirmation.
...
1. `delegate_agent` the spec to `tech-lead` with a brief that asks for: one self-contained HTML file at `demos/demo.html` (canonical workspace copy) plus the `/api/media/` delivery link via the `code_executor` OUTPUT_DIR mechanism, followed by the read-only static self-check from the skill's Rule 4. The brief must be self-sufficient — the sub-agent sees the conversation, but state the deliverable and the acceptance criteria explicitly — and should ask for a **compact verdict line** (link + self-check result) rather than the file body: the result comes back as the sub-agent's reply, capped at 4000 characters.
2. Tech Lead reads `agents/skills/generation/demo-maker.md`, generates, self-checks, and the delegation result carries the delivery link and check results back to you.
...
1. `delegate_agent` the verification to `quality-assurance`: verify `demos/demo.html` with Playwright — load `file://.../demos/demo.html` via an inline `code_executor` python-playwright script (no residue files — never `tests/generated/`, that is the Jira-card automation area); assert no console errors, no page errors, no failed requests; click the primary interactive controls and assert state changes; take a desktop-viewport screenshot written to OUTPUT_DIR (auto-delivered as evidence). Report pass/fail with concrete errors and screenshots.
2. **Fix loop** — on failure, `delegate_agent` the failure list back to `tech-lead` (overwrite `demos/demo.html` in place, re-run the self-check, re-deliver), then delegate re-verification to QA. At most two fix rounds; if still failing, escalate the issue list to the user. Every round is one more delegation inside the same turn — do not hand the turn back to the user between rounds.
