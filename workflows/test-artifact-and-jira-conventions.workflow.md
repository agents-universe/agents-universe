...
- Recordings come from one of two places and both are scenario evidence: an interactive session (`browser_playwright(operation="record_start")` … `record_stop(filename="<case-id>-<scenario-slug>")`, landing in `.tmp/media/{conversation_id}/`) or a server-side spec run (`tests/test-results/**/video.webm`, kept for **failed** cases only — the scaffold uses `retain-on-failure`).
- Name a recording when it is recorded. `record_stop(filename=...)` is the only chance to set it — nothing downstream can rename a file, and that name is what a reviewer sees on the Jira attachment. A recording that is never stopped in the same turn is discarded. A scenario with no scenario-named recording is not complete evidence: record one (or rerun the failing spec) rather than uploading a generic `video.webm`.
...
- If a scenario uses file upload, the exact original input file used by the test is mandatory evidence and must be uploaded to the corresponding Jira test card. Keep the filename scenario-meaningful and make clear that it is the upload input file. When the payload was generated in memory (`browser_playwright(operation="upload", files=[{source: "inline", ...}])`, a spec's `Buffer.from(...)`, or an `api_request(files=[...])` call) there is no file on disk to attach: write the exact bytes to a scenario-named file first (e.g. `tests/generated/artifacts/<issue>/<case-id>-<scenario-slug>-input.csv`) and attach that. Never hand-write a later approximation of the payload — Jira must receive the bytes the system actually got.
- If a scenario calls a multipart API, the exact form fields and the file parts sent are mandatory evidence, same as a JSON request body — save the request contract as a `.json` evidence file (no secrets) plus the file parts themselves.
